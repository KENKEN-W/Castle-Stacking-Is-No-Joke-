    ...
    for(let seed=1;seed<=boards;seed++) {
      ...
      for(const policy of ["number","color"]) {
        ...
        for(const current of deck) {
          ...
          if(G.isVictory(columns))break;
        }
        const s=G.score(columns);m.castles+=s.castles;m.points+=s.points;m.reached15+=s.castles>=15?1:0;m.mono+=s.mono>0?1:0;m.wins+=G.isVictory(columns)?1:0;m.opportunities+=opportunity?1:0;
      }
    }
    return {boards, seeds:`1..${boards}`, definition:"機会＝同じ観測盤面で数字優先と同色優先の配置先が異なる場面。完成や得点の保証ではない。再合戦・勝ち戦後の継続は含まない独立盤面比較。",metrics};
  }
