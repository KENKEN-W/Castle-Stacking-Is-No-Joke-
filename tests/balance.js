  ...
  function run(boards=10000) {
    ...
    for(let seed=1;seed<=boards;seed++) {
      ...
      for(const policy of ["number","color"]) {
        ...
        const s=G.score(columns);m.castles+=s.castles;m.points+=s.points;m.reached15+=s.castles>=15?1:0;m.mono+=s.mono>0?1:0;m.wins+=s.castles===25?1:0;m.opportunities+=opportunity?1:0;
      }
    }
