(async function () {
  ...
  try {
    ...
    await test('S03 勝ち戦復元・同じ戦の続行',async()=>{await load(F.win());await action('RESUME');assert(doc().querySelector('dialog').textContent.includes('勝ち戦！'));assert(stored().battleScore===25);await action('CONTINUE');assert(stored().battle===1&&stored().battleScore===25&&stored().ended===0);});
    ...
    await test('第3戦終了→最終結果→タイトル→再プレイ',async()=>{let s=F.blocked(18);s.battle=3;s.ended=2;s.scores=[21,22,null];await load(s);await action('RESUME');await action('END');assert(stored().finalScore===61);assert(doc().querySelector('.result-number').textContent==='61点');await action('TITLE');assert(text().includes('前回の結果を見る'));await action('RESUME');await action('NEW');assert(stored().battle===1&&stored().wins===0&&stored().best===0);});
