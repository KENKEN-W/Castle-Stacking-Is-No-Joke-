(function () {
  ...
  function showResult() {
    ...
    if (state.phase === G.Phase.WIN) openModal("RESULT", `<div class="confetti" aria-hidden="true">${Array.from({ length: 18 }, (_, i) => `<i style="--i:${i}"></i>`).join("")}</div>${heading(`${info.castles}城 · 同色5段${info.mono}列`, "勝ち戦！")}${breakdown}<p class="result-label">今回の獲得</p><div class="result-number">+${points(info.points)}</div><p class="result-cumulative">第${state.battle}戦 累積 <b>${number(state.battleScore)}点</b></p><p>第${state.battle}戦は続きます。</p>${button("同じ戦を続ける", "CONTINUE")}`, "win-dialog");
    ...
    if (state.phase === G.Phase.GAME_RESULT) {
      const rank = G.playerRank(state.finalScore);
      openModal("RESULT", `${heading("五段重ね将軍", "戦績")}<div class="result-rows">${state.scores.map((n, i) => `<p><span>第${i + 1}戦</span><b>${number(n)}点</b></p>`).join("")}</div><p class="result-label">最終得点</p><div class="result-number">${points(state.finalScore)}</div><div class="rank-card"><span>あなたの評価</span><strong>${rank.name}</strong></div><div class="records"><span>勝ち戦<b>${state.wins}回</b></span><span>再合戦<b>${state.retries}回</b></span><span>最高積載<b>${state.best}城</b></span></div>${button("もう一度遊ぶ", "NEW")}${button("タイトルへ", "TITLE", "quiet")}`);
    }
  }
  function help() {
    openModal("HELP", `${heading("指南書", "遊び方")}<div class="help-content"><h3>一、大きい数字を上へ</h3><p>1〜25の城を5列に積みます。空列には何でも置けます。城の上には、より大きい数字だけ。各列5城までです。</p><div class="help-stack"><span>14</span><span>8</span><span>3</span><small>下から上へ ↑</small></div><p>次に置く城だけが見えます。一度置いた城は移動・取り消しできません。<b>21〜25の城は最初の5手には登場せず、6手目以降に登場します。</b></p><h3>二、まずは15城</h3><div class="help-threshold"><span>14城 <b>0点</b></span><span>15城 <b>15点〜</b></span></div><p>同色5段があっても、15城未満で詰まるとその盤面は0点です。</p><h3>三、色をそろえて倍率アップ</h3><p>赤・青・緑は同じ確率で出現します。1列の5城すべてが同色ならボーナス。列同士の色は違っても構いません。</p><table><caption>同色5段完成列数と倍率</caption><tbody>${C.MONOCHROME_MULTIPLIERS.map((n, i) => `<tr><th>${i}列</th><td>×${n}</td></tr>`).join("")}</tbody></table><p>例：18城・同色5段1列なら、18×2＝36点。色を狙うことで、15城に届かなくなることもあります。</p><h3>四、戦の終わりと続き</h3><ul><li><b>勝ち戦：</b>15城以上を積み、同色5段が1列以上できると突入。得点を加算し、同じ戦を新しい盤面で続けます。</li><li><b>14城以下で詰まる：</b>盤面0点で戦終了。勝ち戦の累積得点は残ります。</li><li><b>15城以上で詰まる：</b>「戦終了」で得点確定、または「再合戦」を選びます。</li><li><b>再合戦：</b>その戦の累積得点もすべて捨てて0点から。同じ戦で何度でも挑戦できます。終了済みの別の戦の得点と参考記録は残ります。</li></ul><p><b>戦終了が3回起きたらゲーム終了。</b>3戦の合計得点に応じて、足軽から天下統一大将軍までの評価を表示します。</p><h3>五、操作と中断</h3><p>列をクリック・タップ、またはキー1〜5。TabとEnter／Spaceでも操作できます。進行は端末内に自動保存され、「つづきから」で再開できます。再読み込みで城の順番は変わりません。</p><p class="small">同じ端末・ブラウザで再開してください。ブラウザの保存データを削除すると進行は失われます。自動保存が利用できない場合は画面に通知します。</p></div>${button("戻る", "CLOSE_HELP")}`, "help-dialog");
  }
