(function (root) {
  ...
  function createDeck(random = Math.random) {
    const openingPool = shuffle(Array.from({ length: C.HIGH_CASTLE_MIN - 1 }, (_, i) => i + 1), random);
    const opening = openingPool.splice(0, C.HIGH_CASTLE_DELAY_DRAWS);
    const high = Array.from({ length: TOTAL - C.HIGH_CASTLE_MIN + 1 }, (_, i) => C.HIGH_CASTLE_MIN + i);
    const numbers = opening.concat(shuffle(openingPool.concat(high), random));
  ...
  const isVictory = columns => {
    const info = score(columns);
    return info.castles >= C.MIN_SCORE_CASTLES && info.mono > 0;
  };
  const playerRank = points => C.PLAYER_RANKS.find(rank => points >= rank.min);
  ...
  function settlePlacement(state) {
    ...
    state.current = state.remaining.shift() || null;
    if (!state.current || !available(state).length) {
  ...
  const Game = { TOTAL, Phase, clone, count, isMono, score, randomColor, createDeck, newGame, canPlace, available, isVictory, playerRank, totalScore, transition };
